...
function queryLastEvent()
{
    ...
    // fetch all guests
    $stmt = $conn->prepare("SELECT guest_id, name, table_number, num_guests, new_table_number, new_num_guests, new_arrival_time, new_handled_by, comments FROM guests WHERE event_id=?");
    $stmt->bind_param("i", $event->event_id);
    $stmt->execute();
    $guest = new stdClass();
    $stmt->bind_result($guest->guest_id, $guest->name, $guest->table_number, $guest->num_guests, $guest->new_table_number, $guest->new_num_guests, $guest->new_arrival_time, $guest->new_handled_by, $guest->comments);
    $guests = array();
    while ($stmt->fetch()) {
        $guests[] = $guest;
        $guest = new stdClass();
...
function submitGuest()
{
    ...
    $result = null;
    $stmt = $conn->prepare('UPDATE guests SET new_table_number = ?, new_num_guests = ?, new_arrival_time = ?, new_handled_by = ?, comments = ? WHERE guest_id = ?');
    $stmt->bind_param("sisssi", $newTableNumber, $newNumGuests, $newArrivalTime, $newHandledBy, $comments, $guestId);
    $stmt->execute();
    $stmt->close();
